...
void app_main() {
 ...
 // CONFIGURACIÓN DEL ADC
    ...
    adc_oneshot_chan_cfg_t config = {
        .atten = ADC_ATTEN_DB_12,   // 0–3.3V (IMPORTANTE)
  ...
  //PWM
    ...
    while(1){
        ...
        if (timer_value >= SAMPLE_PERIOD_US && direccion ==0) { //comparo

            adc_oneshot_read(adc1_handle, ADC_CHANNEL_6, &adc_raw); //leo el valor del potenciometro

            int porcentaje = 100 - ((adc_raw * 100) / 4095); // hago la super conversion de la vida

            value1 = porcentaje / 100;
            value2 = (porcentaje / 10) % 10;
            value3 = porcentaje % 10; //saco los values uwuwuwuuw
